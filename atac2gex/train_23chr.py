...
args = Namespace(
    ...
    train_mod2_cajal=f'{dataset_path}train_mod2_simple.h5ad',
    pretrain='../pretrain/',
...
now = datetime.now()
# time_train = now.strftime("%d_%m_%Y-%H_%M_%S") + f'-{mod1_name}2{mod2_name}-23chr'
...
for chr in range(1, 23):
    ...
    if not os.path.exists(f'{args.save_model_path}{time_train}/chr{str(chr)}'):
